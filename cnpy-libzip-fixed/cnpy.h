...
#include "extern/libzippp/zip.hpp"
...
namespace cnpy {
    ...
    template <typename T>
    uint64_t add_npy_to_zip(libzip::archive &zip, std::string fname, const T *data,
    ...
    void npz_save(std::string zipname, std::string fname, const T *data,
        ...
        //   // Write everything
        uint64_t index = add_npy_to_zip(zip, fname, data, shape);
